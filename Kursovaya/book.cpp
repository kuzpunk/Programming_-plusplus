...
// sort books  
// Функция для сортировки и вывода отсортированного списка книг
void sort_books(const std::unordered_map<int, Book>& booksMap, const std::string& sortBy) {
    // Скопировать данные из unordered_map в vector для сортировки
    vector<pair<int, Book>> booksVector(booksMap.begin(), booksMap.end());

    // Выполнение сортировки вектора на основе выбранного критерия
    if (sortBy == "author") {
        sort(booksVector.begin(), booksVector.end(), 
            [](const pair<int, Book>& a, const pair<int, Book>& b) {
                return a.second.author < b.second.author;
            });
        ...
        sort(booksVector.begin(), booksVector.end(), 
            [](const pair<int, Book>& a, const pair<int, Book>& b) {
                return a.second.category < b.second.category; // Убедитесь, что enum можно сравнивать
            });
    } else {
    ...
    cout << "\nSorted list of books by " << sortBy << ":\n";
    cout << left;
    cout << setw(10) << "ID" << setw(25) << "Author" << setw(30) << "Title" 
         << setw(10) << "Stock" << setw(10) << "Price" << setw(10) << "Year" 
         << setw(15) << "Category" << endl;
    ...
    for (const auto& pair : booksVector) {
        print_book(pair.second);
    }
}
